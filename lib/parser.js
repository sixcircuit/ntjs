...
const keywords_class = require("./keywords.js");
...
var keywords = new keywords_class({
   runtime: ["waiter"],
   blocks: ["tame", "await"],
...
function get_declaration(node){
   ...
   var name = node.name;
   if(!keywords.is_token(name)){ return(null); }

   var token = keywords.out[name];
   token = token.split(" ");
   let type = token[0];
   let raw = (token[1] === "raw");
...
function defer_results_lhs(call){
   ...
   if(call.arguments.length === 0){ return({ lhs: null }); }
      ...
      if(assign.lhs){
         ...
         if(assign.unzip){
            ...
            ast = make_ast(`(async function(){ 
               ${assign.lhs} = ${keywords.in.waiter}.unzip((await ${waiter_id}.promise()));
            })();`);
            */
...
const transform_visitor = {
   WhileStatement(path, state){
      if(!types.isIdentifier(path.node.test, { name: keywords.in.await })){ return; }
      ...
      let create_waiter = make_ast(`const ${waiter_id} = ${keywords.in.waiter}();`);
...
function tame_source(code, options){
   ...
   var runtime_ast = make_ast(`
      let ${keywords.in.waiter} = null;
      if(typeof require !== undefined){ ${keywords.in.waiter} = require('tamejs'); }else{ ${keywords.in.waiter} = window.${keywords.in.waiter}; }
   `);
