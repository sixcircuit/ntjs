...
function defer_results_lhs(call){
   ...
   var lhs = declaration_type + "[" + args.join(",") + "]";

   return({ lhs, unzip, raw });
}
...
function handle_results_ast(results_id, waiter_id, defer_calls){
      ...
      if(lhs){
         if(lhs.raw){
            ast = make_ast(`(async function(){ 
               ${lhs.lhs} = (await ${waiter_id}.promise());
            })();`);
         }else if(lhs.unzip){
            ast = make_ast(`(async function(){ 
               ${lhs.lhs} = __tamejs_waiter.unzip((await ${waiter_id}.promise())[0]);
            })();`);
         }else{
            ast = make_ast(`(async function(){ 
               ${lhs.lhs} = (await ${waiter_id}.promise())[0];
            })();`);
         }
      }else{
         ast = make_ast(`(async function(){ 
   ...
   if(defer_calls.length === 1){
      ...
      ast.push(await_results_ast({ lhs: `const ${results_id}` }));

      _.each(defer_calls, function(call, index){
         let lhs = defer_results_lhs(call);
         if(lhs){
            let assignment_ast = make_ast(`${lhs} = ${results_id}[${index}];`);
