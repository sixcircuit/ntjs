...
tamejs.compile = function(options, callback){
   ...
   const gen = tamejs.parse.file(file.in.path);
   ...
   if(file.out){
      ...
      if(options.map){ 
         ...
         _.fs.write_file.sync(file.map.path, gen.map);
      }

      if(options.stages){
         file.stages = {};

         _.each(gen.stages, function(stage, { key }){
            const stage_file = _.path.extension(file.out.path, key + ".js", "js");
            _.fs.write_file.sync(stage_file, stage);
            file.stages[key] = { path: stage_file, root: file.out.root, name: _.path.file(stage_file) };
         });
      }
   }
...
tamejs.require = function(options){
   ...
   return tamejs.compile({ in: file.in.path, out: file.cache ? file.cache.path : null });
};
...
tamejs.register = function(options){
   ...
   require.extensions["." + ext] = function(module, file_path){
      const gen = tamejs.require(_.merge(options, { in: file_path }));
