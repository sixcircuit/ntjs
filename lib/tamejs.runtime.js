
(function(){

   function waiter(){
      this._pending = 0;
      this._callback = null;
      this._results = [];
   }

   waiter.prototype.cb = function(i){
      var self = this;
      self._pending++;
      var once = false;
      return(function(){
         if(once){ return; } once = true;
         self._pending--;
         if(i !== undefined){
            self._results[i] = arguments;
         }
         self._try_finish();
      });
   };

   waiter.prototype._try_finish = function(){
      var self = this;
      if(self._pending !== 0){ return; }

      var no_callback = 0;
      (function finish(){
         if(self._callback){ return self._callback(self._results); }
         else{ 
            no_callback++;
            if((no_callback % 10) == 0){ 
               console.log("error: tamejs: no callback to call back: " + no_callback + " times. we'll keep nextTick'n till we get one.\n" + (new Error()).stack); 
               throw (new Error("error: tamejs: no callback to call back: " + no_callback + " times. we'll keep nextTick'n till we get one.\n"));
            }
            if(typeof process !== undefined){
               process.nextTick(finish);
            }else{
               setTimeout(finish, 1);
            }
         }
      })();
   };

   waiter.prototype.promise = function(){
      var self = this;
      return({ then: function(cb, err_cb){ 
         self._callback = cb;
         self._try_finish();  // if cb() is never called, make sure we don't hang
      } });
   };

   function factory(){ return(new waiter()); }

   if(typeof require === undefined){ window.__tamejs_waiter = factory; }else{ module.exports = factory; }

})();

